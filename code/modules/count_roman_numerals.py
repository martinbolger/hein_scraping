...
def count_roman_numerals(first_page, last_page):
    ...
    if bool(re.search(r"(^(?=[MDCLXVI])M*(C[MD]|D?C{0,3})(X[CL]|L?X{0,3})(I[XV]|V?I{0,3})$)", first_page, flags = re.I)) == True:
        count += 1
    if bool(re.search(r"(^(?=[MDCLXVI])M*(C[MD]|D?C{0,3})(X[CL]|L?X{0,3})(I[XV]|V?I{0,3})$)", last_page, flags = re.I)) == True:
        count += 1
